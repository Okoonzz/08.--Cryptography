#ifndef HAES_H
#define HAES_H

#include <vector>
#include <algorithm>
class AES
{

    std::vector<unsigned char> S_BOX = {
        };

    std::vector<std::vector<unsigned char>> RCON = {
        {0x01, 0x00, 0x00, 0x00},
        {0x02, 0x00, 0x00, 0x00},
        {0x04, 0x00, 0x00, 0x00},
        {0x08, 0x00, 0x00, 0x00},
        {0x10, 0x00, 0x00, 0x00},
        {0x20, 0x00, 0x00, 0x00},
        {0x40, 0x00, 0x00, 0x00},
        {0x80, 0x00, 0x00, 0x00},
        {0x1B, 0x00, 0x00, 0x00},
        {0x36, 0x00, 0x00, 0x00}};

    std::vector<unsigned char> INV_S_BOX = {
        };

public:
    std::vector<std::vector<unsigned char>> round_keys;

    std::vector<unsigned char> sub_word(std::vector<unsigned char> word)
    {
        std::vector<unsigned char> result;
        for (auto b : word)
        {
            result.push_back(S_BOX[b]);
        }
        return result;
    }

    std::vector<unsigned char> rot_word(std::vector<unsigned char> word)
    {
        std::rotate(word.begin(), word.begin() + 1, word.end());
        return word;
    }

    std::vector<std::vector<unsigned char>> key_expansion_128(std::string key)
    {
        unsigned char key_size = 16;
        unsigned char key_words = 4;
        std::vector<unsigned char> keyInts;

        for (char &c : key)
        {
            keyInts.push_back(static_cast<unsigned char>(c));
        }

        for (unsigned char i = 0; i < key_size; i += 4)
        {
            round_keys.push_back(std::vector<unsigned char>(keyInts.begin() + i, keyInts.begin() + i + 4));
        }

        for (unsigned char i = key_words; i < 44; ++i)
        {
            std::vector<unsigned char> temp = round_keys[i - 1];
            if (i % key_words == 0)
            {
                temp = sub_word(rot_word(temp));
                for (unsigned char j = 0; j < 4; ++j)
                {
                    temp[j] ^= RCON[(i - key_words) / key_words][j];
                }
            }
            std::vector<unsigned char> new_word;
            for (unsigned char j = 0; j < 4; ++j)
            {
                new_word.push_back(round_keys[i - key_words][j] ^ temp[j]);
            }
            round_keys.push_back(new_word);
        }
        return round_keys;
    }

    std::vector<std::vector<unsigned char>> sub_bytes(std::vector<std::vector<unsigned char>> &state)
    {
        for (unsigned char i = 0; i < 4; ++i)
        {
            for (unsigned char j = 0; j < 4; ++j)
            {
                unsigned char row = state[i][j] / 0x10;
                unsigned char col = state[i][j] % 0x10;
                state[i][j] = S_BOX[16 * row + col];
            }
        }
        return state;
    }

    std::vector<std::vector<unsigned char>> shift_rows(std::vector<std::vector<unsigned char>> &state)
    {
        std::rotate(state[1].begin(), state[1].begin() + 1, state[1].end());

        std::rotate(state[2].begin(), state[2].begin() + 2, state[2].end());

        std::rotate(state[3].begin(), state[3].begin() + 3, state[3].end());
        return state;
    }

    unsigned char gmul(unsigned char a, unsigned char b)
    {
        unsigned char p = 0;
        for (unsigned char counter = 0; counter < 8; counter++)
        {
            if ((b & 1) != 0)
            {
                p ^= a;
            }
            bool hi_bit_set = (a & 0x80) != 0;
            a <<= 1;
            if (hi_bit_set)
            {
                a ^= 0x1b;
            }
            b >>= 1;
        }
        return p % 256;
    }

    std::vector<std::vector<unsigned char>> mix_columns(std::vector<std::vector<unsigned char>> &state)
    {
        for (unsigned char i = 0; i < 4; ++i)
        {
            std::vector<unsigned char> col(4);
            for (unsigned char j = 0; j < 4; ++j)
            {
                col[j] = state[j][i];
            }
            state[0][i] = gmul(0x02, col[0]) ^ gmul(0x03, col[1]) ^ col[2] ^ col[3];
            state[1][i] = col[0] ^ gmul(0x02, col[1]) ^ gmul(0x03, col[2]) ^ col[3];
            state[2][i] = col[0] ^ col[1] ^ gmul(0x02, col[2]) ^ gmul(0x03, col[3]);
            state[3][i] = gmul(0x03, col[0]) ^ col[1] ^ col[2] ^ gmul(0x02, col[3]);
        }
        return state;
    }
    std::vector<std::vector<unsigned char>> inv_mix_columns(std::vector<std::vector<unsigned char>> &state)
    {
        for (unsigned char i = 0; i < 4; ++i)
        {
            std::vector<unsigned char> col(4);
            for (unsigned char j = 0; j < 4; ++j)
            {
                col[j] = state[j][i];
            }
            state[0][i] = gmul(0x0E, col[0]) ^ gmul(0x0B, col[1]) ^ gmul(0x0D, col[2]) ^ gmul(0x09, col[3]);
            state[1][i] = gmul(0x09, col[0]) ^ gmul(0x0E, col[1]) ^ gmul(0x0B, col[2]) ^ gmul(0x0D, col[3]);
            state[2][i] = gmul(0x0D, col[0]) ^ gmul(0x09, col[1]) ^ gmul(0x0E, col[2]) ^ gmul(0x0B, col[3]);
            state[3][i] = gmul(0x0B, col[0]) ^ gmul(0x0D, col[1]) ^ gmul(0x09, col[2]) ^ gmul(0x0E, col[3]);
        }
        return state;
    }

    std::vector<std::vector<unsigned char>> add_round_key(std::vector<std::vector<unsigned char>> &state, unsigned char round_number)
    {
        for (unsigned char i = 0; i < 4; ++i)
        {
            for (unsigned char j = 0; j < 4; ++j)
            {
                state[j][i] ^= round_keys[round_number * 4 + i][j];
            }
        }
        return state;
    }

    std::vector<std::vector<unsigned char>> inv_sub_bytes(std::vector<std::vector<unsigned char>> &state)
    {
        for (unsigned char i = 0; i < 4; ++i)
        {
            for (unsigned char j = 0; j < 4; ++j)
            {
                unsigned char row = state[i][j] / 0x10;
                unsigned char col = state[i][j] % 0x10;
                state[i][j] = INV_S_BOX[16 * row + col];
            }
        }
        return state;
    }

    std::vector<std::vector<unsigned char>> inv_shift_rows(std::vector<std::vector<unsigned char>> &state)
    {
        std::rotate(state[1].rbegin(), state[1].rbegin() + 1, state[1].rend());

        std::rotate(state[2].rbegin(), state[2].rbegin() + 2, state[2].rend());

        std::rotate(state[3].rbegin(), state[3].rbegin() + 3, state[3].rend());

        return state;
    }

    std::vector<unsigned char> encrypt(std::vector<unsigned char> data, std::string key)
    {
        
        
        if (round_keys.empty())
        {
            round_keys = key_expansion_128(key);
        }

      
        std::vector<std::vector<unsigned char>> state(4);
        // for (std::vector<unsigned char>::size_type i = 0; i < data.size(); i += 4)
        // {
        //     state.push_back(std::vector<unsigned char>(data.begin() + i, data.begin() + i + 4));
        // }
        for (std::vector<unsigned char>::size_type i = 0; i < data.size(); i++) {
            state[i % 4].push_back(data[i]);
        }
        
        unsigned char num_rounds = (key.size() == 16) ? 10 : (key.size() == 24) ? 12 : 14;

        add_round_key(state, 0);
       
        for (unsigned char round = 1; round < num_rounds; ++round)
        {
            sub_bytes(state);
            shift_rows(state);
            mix_columns(state);
            add_round_key(state, round);
        }

        sub_bytes(state);
        shift_rows(state);
        add_round_key(state, num_rounds);

        
        // std::vector<unsigned char> result;
        // for (auto &row : state)
        // {
        //     result.insert(result.end(), row.begin(), row.end());
        // }
        std::vector<unsigned char> result;
        for (size_t i = 0; i < state[0].size(); ++i) {
            for (auto &row : state) {
                if (i < row.size()) {
                result.push_back(row[i]);
                }
            }
        }

        return result;
    }

    std::vector<unsigned char> decrypt(std::vector<unsigned char> ciphertext, std::string key)
    {
        if (round_keys.empty())
        {
            round_keys = key_expansion_128(key);
        }

        std::vector<std::vector<unsigned char>> state(4);
        // for (std::vector<unsigned char>::size_type i = 0; i < ciphertext.size(); i += 4)
        // {
        //     state.push_back(std::vector<unsigned char>(ciphertext.begin() + i, ciphertext.begin() + i + 4));
        // }
        for (std::vector<unsigned char>::size_type i = 0; i < ciphertext.size(); i++) {
            state[i % 4].push_back(ciphertext[i]);
        }

        unsigned char num_rounds = (key.size() == 16) ? 10 : (key.size() == 24) ? 12 : 14;

        add_round_key(state, num_rounds);

        for (unsigned char round = num_rounds - 1; round > 0; --round)
        {
            inv_shift_rows(state);
            inv_sub_bytes(state);
            add_round_key(state, round);
            inv_mix_columns(state);
        }

        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, 0);

        std::vector<unsigned char> result;
        for (size_t i = 0; i < state[0].size(); ++i) {
            for (auto &row : state) {
                if (i < row.size()) {
                result.push_back(row[i]);
                }
            }
        }

        return result;
    }
};

#endif
